...
ALLOCATE(M%VEG_DRAG(0:IBP1,0:JBP1,1:10),STAT=IZERO) ; CALL ChkMemErr('INIT','VEG_DRAG',IZERO); M%VEG_DRAG = 0._EB

VEG_LEVEL_SET: IF (VEG_LEVEL_SET) THEN
  ALLOCATE(M%LSET_PHI(0:IBP1,0:JBP1),STAT=IZERO) ; CALL ChkMemErr('INIT','LSET_PHI',IZERO) ; M%LSET_PHI = -1._EB
  ...
  ALLOCATE(M%CFB_LS(0:IBP1,0:JBP1)) ; CALL ChkMemErr('INIT','CFB_LS',IZERO) ; M%CFB_LS= 0._EB
...
IF (ANY(MEAN_FORCING) .AND. .NOT.EVACUATION_ONLY(NM)) THEN
   DO IW=1,M%N_EXTERNAL_WALL_CELLS
      WC => M%WALL(IW)
      IOR = WC%ONE_D%IOR
      IF (WC%BOUNDARY_TYPE==OPEN_BOUNDARY .AND. MEAN_FORCING(ABS(IOR))) M%APPLY_SPONGE_LAYER(IOR) = .TRUE.
   ENDDO
ENDIF

! Find exterior wall cells of HT3D solids

CALL ASSIGN_HT3D_WALL_INDICES(NM)
...
IF (ANY_REMOVE_OBST .AND. SOLID_PYRO3D) CALL ASSIGN_HT3D_WALL_INDICES(NM)
